...
describe('domain fixtures', () => {
  it('include all event variants with event identity and timestamps', () => {
    ...
    expect(eventTypes).toEqual(['seizure', 'observation', 'therapy_dog']);
  ...
  it('represent canonical variant fields', () => {
    ...
    expect(therapyDogFixture.intensity).toBe('medium');
    expect(therapyDogFixture.durationMinutes).toBe(120);
    expect(therapyDogFixture.tags).toContain('Krankenhaus');
  });
...
describe('domain type constraints', () => {
  it('exposes controlled score unions', () => {
    expectTypeOf<'green'>().toMatchTypeOf<ColorScore>();
    expectTypeOf<'trigger-tag'>().toMatchTypeOf<KnownTermKind>();
  });
  ...
  it('supports exhaustive event variant switching', () => {
    const labels = allEventVariantFixtures.map((event): string => {
      switch (event.type) {
        ...
        case 'therapy_dog':
          return event.tags[0] ?? 'therapy_dog';
        default:
          return assertNever(event);
      }
    });

    expect(labels).toEqual(['medium', 'restless', 'Krankenhaus']);
  });
});
